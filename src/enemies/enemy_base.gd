# enemy_base.gd — Single shared enemy AI system used by every enemy type
# across all missions.  Handles: player tracking, dormant/alert state,
# freeze/burn status effects, armor + damage resolution, line of sight,
# A* obstacle pathfinding and prioritized behavior execution.
#
# Subclasses only supply behaviors (via _create_behaviors) and cosmetic hooks.
...
## A* pathfinding tuning (ground units that steer around obstacles)
...
## Ordered list of behaviors.  Each takes (delta: float) and returns true if it
## handled the frame (sets velocity / fires); first true short-circuits.
func _create_behaviors() -> Array[Callable]:
	return []

## Extra setup after base _ready.
func _enemy_ready() -> void:
	pass

## Per-frame timer bookkeeping while alerted and unfrozen.
func _update_timers(_delta: float) -> void:
	pass

## Resting color the damage flash tweens back to.
func _visual_base_color() -> Color:
	return Color.WHITE

## Cosmetic effect spawned on death (blood, scrap, explosion...).
...
func _flash_damage() -> void:
	...
	tween.tween_property(_visual, "color", _visual_base_color(), 0.1)
...
## Generic ground chase: walk straight at the player with line of sight,
## otherwise A*-path around environment obstacles.  Always returns true.
func _behavior_chase_player(_delta: float) -> bool:
